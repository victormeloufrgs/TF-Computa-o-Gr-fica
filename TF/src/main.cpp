...
glm::vec4 getPositionAnimationBezierCubic(glm::vec4 cp1, glm::vec4 cp2, glm::vec4 cp3, glm::vec4 cp4, float timeInSec);
bool colidiram(glm::vec4 center1, float y1_size, float x1_size, float z1_size, glm::vec4 center2, float y2_size, float x2_size, float z2_size);
bool isBetween(glm::vec4 point, glm::vec4 min_point, glm::vec4 max_point);
...
float jumping_total_time = 1.0f;
float jumping_current_time = 0.0f;
float jumping_start_seconds = 0.0f;
float jumping_height = 2.0f;
float jumping_front_distance = 1.0f;
glm::vec4 jumping_source;
glm::vec4 jumping_destiny;
glm::vec4 jumping_c2;
glm::vec4 jumping_c3;
bool is_jumping = false;


float altura_personagem = 3.0f;

glm::vec4 camera_position_c = glm::vec4(0.0f,altura_personagem-1,3.0f,1.0f);
glm::vec4 camera_view_vector = glm::vec4(0.0f,-0.5f,-1.0f,0.0f);
glm::vec4 camera_up_vector   = glm::vec4(0.0f,1.0f,0.0f,0.0f); // Vetor "up" fixado para apontar para o "c�u" (eito Y global)

float mouse_dx = 0.0f;
float mouse_dy = 0.0f;

glm::vec4 last_camera_view_vector_temp;
...
int main(int argc, char* argv[])
{
  ...
  //  LoadTextureImage("../../data/floor.jpg");      // TextureImage0
    ...
    ObjModel weaponmodel("../../data/weapon/weapon_low_poly.obj");
    ComputeNormals(&weaponmodel);
    BuildTrianglesAndAddToVirtualScene(&weaponmodel);

    //MODELO INIMIGO
    ObjModel inimigomodel("../../data/inimigo_iron.obj");
    ComputeNormals(&inimigomodel);
    BuildTrianglesAndAddToVirtualScene(&inimigomodel);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // uso o segundo para controlar (deixar mais lento) a anima��o de movimenta��o
        // quando as matrizes n�o estiverem sendo mostradas.
        float currentSecond = (float)glfwGetTime();
        float ellapsed_seconds = currentSecond - lastSecond;

        glm::vec4 strf_direction = crossproduct(camera_view_vector,camera_up_vector);

        glm::vec4 old_camera_position_c = camera_position_c;

        float speed = 5.0;
        if (!is_jumping) {
            if(g_WKeyPressed) {
              camera_position_c += ellapsed_seconds * camera_view_vector * speed;
            }
            ...
            if(g_AKeyPressed) {
                ...
                camera_position_c -= ellapsed_seconds * strf_direction * speed;
            }
            ...
            if(g_SpaceKeyPressed) {
                jumping_current_time = currentSecond;
                is_jumping = true;
                jumping_start_seconds = currentSecond;

                jumping_source = camera_position_c;

                jumping_destiny = camera_position_c;
                jumping_destiny.x += camera_view_vector.x*2.0f;
                jumping_destiny.z += camera_view_vector.z*2.0f;

                jumping_c2 = camera_position_c + jumping_destiny*1.0f/4.0f;
                jumping_c2.y += jumping_height;

                jumping_c3 = camera_position_c + (jumping_destiny - jumping_destiny*1.0f/4.0f);
                jumping_c3.y += jumping_height;
            }
        } else { // is jumping

            if(jumping_current_time >= jumping_start_seconds+jumping_total_time) {
                is_jumping = false;
            }

            if(currentSecond - jumping_current_time >= 1) {
                jumping_current_time += 1;
            }

            camera_position_c = getPositionAnimationBezierCubic(jumping_source, jumping_c2, jumping_c3, jumping_destiny, currentSecond-jumping_start_seconds);
        ...
        glm::vec4 chao = glm::vec4(0.0f, -1.0f, 0.0f, 1.0f);

        if (colidiram(camera_position_c,altura_personagem*2,1,1,chao,2,10000,10000)) {
            //std::cout << "colidiu" << std::endl;
            camera_position_c.y = old_camera_position_c.y;
        } else {
            if(camera_position_c.y > altura_personagem-1.0f) {
                camera_position_c.y -= 0.1; // gravidade
            } else if (camera_position_c.y < altura_personagem-1.0f) {
                camera_position_c.y = altura_personagem-1;
            }
        }
        ...
        // definir o sistema de coordenadas da c�mera.  Veja slide 186 do documento "Aula_08_Sistemas_de_Coordenadas.pdf".
        ...
        // est�o no sentido negativo! Veja slides 190-193 do documento "Aula_09_Projecoes.pdf".
        ...
        {
            // Proje��o Perspectiva.
            // Para defini��o do field of view (FOV), veja slide 227 do documento "Aula_09_Projecoes.pdf".
        ...
        else
        {
            ...
            // PARA PROJE��O ORTOGR�FICA veja slide 236 do documento "Aula_09_Projecoes.pdf".
       ...
       // glm::vec4 perp_cam_view = camera_view_vector
        glm::vec4 weap_pos = camera_position_c;// + camera_view_vector * 2.0f;
        glm::vec4 inimigo_pos = camera_position_c;// + camera_view_vector * 2.0f;

        // Desenhamos o modelo da arma

        glm::vec4 vec_x = glm::vec4(1.0f,0.0f,0.0f,0.0f);
        glm::vec4 vec_z = glm::vec4(0.0f,1.0f,0.0f,0.0f);

        float rotation_y = dotproduct(vec_x, camera_view_vector);
        float rotation_x = dotproduct(vec_z, camera_view_vector);

        model = Matrix_Translate(weap_pos.x, weap_pos.y, weap_pos.z)
                * Matrix_Rotate_Y(g_CameraTheta)
                * Matrix_Rotate_X(-g_CameraPhi)
                * Matrix_Scale(0.5,0.5,0.5)
                * Matrix_Rotate_Y(3.14159)
                * Matrix_Rotate_X(3.14159/10.0)
                * Matrix_Translate(-1.2f,-2.0f,2.0f);
        glUniformMatrix4fv(model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
        glUniform1i(object_id_uniform, WEAPON);
        DrawVirtualObject("weapon");

        // Desenhamos o modelo do inimigo  // não ta funcionando
        model = Matrix_Translate(inimigo_pos.x, inimigo_pos.y, inimigo_pos.z)
              //  * Matrix_Rotate_Y(g_CameraTheta)
              //  * Matrix_Rotate_X(g_CameraPhi)
                * Matrix_Scale(2,2,2)
        ...
        DrawVirtualObject("inimigo_iron");


        // Desenhamos o modelo do plano
        model = Matrix_Translate(0.0f,-1.0f,0.0f)* Matrix_Scale(10,1,10);
        glUniformMatrix4fv(model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
...
// utilizados para renderiza��o. Veja slides 217-219 do documento "Aula_03_Rendering_Pipeline_Grafico.pdf".
//
...
void ComputeNormals(ObjModel* model)
{
    ...
    for (size_t shape = 0; shape < model->shapes.size(); ++shape)
    {
        ...
        for (size_t triangle = 0; triangle < num_triangles; ++triangle)
        {
            ...
            const glm::vec4 bc = c-b;

            const glm::vec4  n = crossproduct(ab,bc);
...
void CursorPosCallback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    mouse_dx = xpos - g_LastCursorPosX;
    mouse_dy = ypos - g_LastCursorPosY;


    // resolve ultra sensibilidade na rotação vertical
    float limit_y_rotation = 20;
    if (mouse_dy < -limit_y_rotation) {
        mouse_dy = -limit_y_rotation;
    } else if (mouse_dy > limit_y_rotation) {
        mouse_dy = limit_y_rotation;
    }

    g_CameraTheta -= 0.01f*mouse_dx;
    g_CameraPhi   += 0.01f*mouse_dy;

    // rotaciona a c�mera
    const float ROTATION_SPEED = 0.01f;
    camera_view_vector = Matrix_Rotate(-mouse_dx*ROTATION_SPEED,camera_up_vector)*camera_view_vector;

    glm::vec4 toRotateAround = crossproduct(camera_view_vector,camera_up_vector);
    glm::vec4 camera_view_vector_temp = Matrix_Rotate(-mouse_dy*ROTATION_SPEED,toRotateAround)*camera_view_vector;

    // corrige bug ao girar a c�mera muito pra cima ou muito pra baixo
    if((last_camera_view_vector_temp.y < -0.99f && mouse_dy > 0.0f) || (last_camera_view_vector_temp.y > 0.99f && mouse_dy < 0.0f)) {
        camera_view_vector_temp = Matrix_Rotate(0.0f*ROTATION_SPEED,toRotateAround)*camera_view_vector;
        g_CameraPhi   -= 0.01f*mouse_dy;
    }

    last_camera_view_vector_temp = camera_view_vector_temp;
    camera_view_vector = camera_view_vector_temp;
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    // Minha implementa��o (Victor)

    // Se o usu�rio apertar a tecla W, move a c�mera para frente
    if (key == GLFW_KEY_W) {
    ...
    // Se o usu�rio apertar a tecla S, move a c�mera para tr�s
    if (key == GLFW_KEY_S) {
        if (action == GLFW_PRESS) {
            g_SKeyPressed = true;
        } else if (action == GLFW_RELEASE){
    ...
    // Se o usu�rio apertar a tecla A, move a c�mera para esquerda
    if (key == GLFW_KEY_A) {
        if (action == GLFW_PRESS) {
            g_AKeyPressed = true;
        } else if (action == GLFW_RELEASE){
    ...
    // Se o usu�rio apertar a tecla D, move a c�mera para direita
    if (key == GLFW_KEY_D) {
        if (action == GLFW_PRESS) {
            g_DKeyPressed = true;
        } else if (action == GLFW_RELEASE){
    ...
    // Se o usuario apertar a tecla espaço, pula.
...
// Detecta colisão entre objetos (cubos ao redor dos objetos)
bool colidiram(glm::vec4 center1, float y1_size, float x1_size, float z1_size, glm::vec4 center2, float y2_size, float x2_size, float z2_size) {

    glm::vec4 lower_point_obj1 = glm::vec4(center1.x-x1_size/2.0f, center1.y-y1_size/2.0f, center1.z-z1_size/2.0f, center1.w);
    glm::vec4 higher_point_obj1 = glm::vec4(center1.x+x1_size/2.0f, center1.y+y1_size/2.0f, center1.z+z1_size/2.0f, center1.w);

    glm::vec4 lower_point_obj2 = glm::vec4(center2.x-x2_size/2.0f, center2.y-y2_size/2.0f, center2.z-z2_size/2.0f, center2.w);
    glm::vec4 higher_point_obj2 = glm::vec4(center2.x+x2_size/2.0f, center2.y+y2_size/2.0f, center2.z+z2_size/2.0f, center2.w);

    if(isBetween(lower_point_obj1, lower_point_obj2, higher_point_obj2) ||
       isBetween(higher_point_obj1, lower_point_obj2, higher_point_obj2) ||
       isBetween(lower_point_obj2, lower_point_obj1, higher_point_obj1) ||
       isBetween(higher_point_obj2, lower_point_obj1, higher_point_obj1)) {
        return true;
    } else {
        return false;
    }
...
bool isBetween(glm::vec4 point, glm::vec4 min_point, glm::vec4 max_point) {
    ...
    if (point.x >= min_point.x && point.y >= min_point.y && point.z >= min_point.z) {
        if (point.x <= max_point.x && point.y <= max_point.y && point.z <= max_point.z) {
            return true;
        }
    }
    return false;
...
glm::vec4 getPositionAnimationBezierCubic(glm::vec4 cp1, glm::vec4 cp2, glm::vec4 cp3, glm::vec4 cp4, float timeInSec) {
    ...
    float c12x = cp1.x + t*(cp2.x - cp1.x);
    float c23x = cp2.x + t*(cp3.x - cp2.x);
    float c34x = cp3.x + t*(cp4.x - cp3.x);

    float c123x = c12x + t*(c23x - c12x);
    float c234x = c23x + t*(c34x - c23x);

    float cx = c123x + t*(c234x - c123x);

    // -----

    float c12y = cp1.y + t*(cp2.y - cp1.y);
    float c23y = cp2.y + t*(cp3.y - cp2.y);
    float c34y = cp3.y + t*(cp4.y - cp3.y);

    float c123y = c12y + t*(c23y - c12y);
    float c234y = c23y + t*(c34y - c23y);

    float cy = c123y + t*(c234y - c123y);

    // ----

    float c12z = cp1.z + t*(cp2.z - cp1.z);
    float c23z = cp2.z + t*(cp3.z - cp2.z);
    float c34z = cp3.z + t*(cp4.z - cp3.z);

    float c123z = c12z + t*(c23z - c12z);
    float c234z = c23z + t*(c34z - c23z);

    float cz = c123z + t*(c234z - c123z);


//      std::cout << "timeInSec" << timeInSec << std::endl;
//    std::cout << "xPos" << xPos << std::endl;
//    std::cout << "yPos" << yPos << std::endl;
//    std::cout << "zPos" << zPos << std::endl;
    return glm::vec4(cx, cy, cz, 1.0f);
